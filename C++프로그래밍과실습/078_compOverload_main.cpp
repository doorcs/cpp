...
int main() {
    ...
    student_grades[1] = {1, "Bob", 90}; // key를 이용해 맵에 접근할 때, 맵에 해당 key가 존재하지 않는다면 맵은 value의 기본 생성자를 호출하기 때문에
    // map[1] = Student(); 
    // map[1] = {1, "Bob", 90}; 사실은  두 단계로 나누어 실행되는 셈이다. 따라서 클래스의 기본 생성자`Student()`가 정의되어 있지 않다면 에러가 발생한다.
